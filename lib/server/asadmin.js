...
module.exports.getAsAdminSettingValues = function (includeAdminPort) {
  return Q.fcall(function () {
    ...
    if (includeAdminPort === true) {
      var portBase = parseInt(config.getConfig('domain.ports.base', 8000), 10);
      var adminPort = parseInt(config.getConfig('domain.ports.admin', -1), 10);
      if (adminPort <= 0) {
        adminPort = portBase + 48;
      }
      values.portBase = parseInt(portBase, 10);
      values.adminPort = parseInt(adminPort, 10);
    }
