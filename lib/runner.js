...
function getAsAdminSettingValues_(includeAdminPort) {
  return Q.fcall(function () {
    ...
    var domainHome = configure.adjustValue(configure.get('domain.home', null));
...
function getMavenSettingValues_() {
  return Q.fcall(function () {
    var messages = [];
    var mavenHome = configure.adjustValue(configure.get('maven.home', null));
    var mavenProject = configure.adjustValue(configure.get('maven.project', 'pom.xml'));
    var mavenSetting = configure.adjustValue(configure.get('maven.setting', null));
...
function getEnvironment_() {
  ...
  _.forEach(process.env, function (value, name) {
    resultEnv[_prepareName(name)] = value;
  });

  if (env) {
    // append the environment from the configuration
    _.forEach(env, function (value, name) {
      resultEnv[_prepareName(name)] = configure.adjustValue(value);
    });
  }

  if (settings.isVerbose()) {
    logger.info('Environment:');
    _.forEach(resultEnv, function (value, name) {
